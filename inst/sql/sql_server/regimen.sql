...
WITH add_groups AS (
    ...
    FROM
        @writeDatabaseSchema. @regimenTable r1
        LEFT JOIN @writeDatabaseSchema.@regimenTable r2 on r1.person_id = r2.person_id
    ...
    SELECT
        ...
        MAX(
            CASE
                ...
                ELSE 0
            END
        ) AS contains_original_ingredient
    FROM
        add_groups g
    GROUP BY
        ingredient_start_date_new,
        person_id
    ORDER BY -- ok to get rid of order by? DB tables are not ordered?
        ingredient_start_date_new
),
regimens_to_keep AS (
    SELECT
        ...
        gs.drug_era_id,
        gs.concept_name,
        rs.ingredient_start_date_new as ingredient_start_date
    FROM
        regimens rs
        LEFT JOIN add_groups gs on rs.person_id = gs.person_id
        and rs.ingredient_start_date_new = gs.ingredient_start_date_new
    WHERE
        contains_original_ingredient > 0
),
updated_table AS (
    SELECT
        *
    ...
    FROM
        @writeDatabaseSchema.@regimenTable
    WHERE
        drug_era_id NOT IN (
            SELECT
                drug_era_id
...
SELECT
    ...
    drug_era_id,
    concept_name,
    ingredient_start_date INTO @writeDatabaseSchema.@regimenTable_tmp
FROM
    updated_table;
...
INSERT INTO
    @writeDatabaseSchema. @regimenTable (
        SELECT
            *
        FROM
            @writeDatabaseSchema. @regimenTable_tmp
    );
